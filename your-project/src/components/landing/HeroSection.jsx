...
import { container, item, hoverLift, smoothTransition, fadeInScale } from "@/lib/motionVariants";

export function HeroSection({ onOpenLogin, onOpenSignup }) {
  const { user, hydrated } = useAuth();
  const router = useRouter();

  const showDashboardButton = hydrated && user;

  return (
    <section className="relative pt-28 pb-16 lg:pt-44 lg:pb-36 overflow-hidden">
      <div className="absolute inset-0 -z-10 overflow-hidden">
        <motion.div
          animate={{ y: [0, -30, 0] }}
          transition={{ duration: 8, repeat: Infinity, ease: "easeInOut" }}
          className="absolute top-0 left-1/4 w-[700px] h-[700px] bg-primary/15 rounded-full blur-[120px]"
        />
        <motion.div
          animate={{ y: [0, 25, 0] }}
          transition={{ duration: 7, repeat: Infinity, ease: "easeInOut", delay: 2 }}
          className="absolute bottom-0 right-1/4 w-[600px] h-[600px] bg-secondary/15 rounded-full blur-[120px]"
        />
        <motion.div
          animate={{ y: [0, -20, 0] }}
          transition={{ duration: 9, repeat: Infinity, ease: "easeInOut", delay: 1 }}
          className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[900px] h-[900px] bg-gradient-to-r from-primary/8 to-secondary/8 rounded-full blur-[150px]"
        />
        <motion.div
          animate={{ y: [0, 35, 0] }}
          transition={{ duration: 10, repeat: Infinity, ease: "easeInOut", delay: 3 }}
          className="absolute top-1/3 left-1/2 -translate-x-1/2 w-[600px] h-[600px] bg-accent/5 rounded-full blur-[100px]"
        />
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <div className="max-w-2xl">
            <motion.div variants={item} className="inline-flex items-center gap-2.5 px-5 py-2.5 rounded-full bg-primary/10 border border-primary/20 mb-10">
            ...
            <motion.h1
              variants={item}
              className="text-4xl sm:text-5xl lg:text-6xl xl:text-7xl font-extrabold text-foreground leading-[1.1] tracking-tight mb-8"
            >
              Recover Lost{" "}
            ...
            <motion.p
              variants={item}
              className="text-base sm:text-lg lg:text-xl text-muted leading-relaxed mb-10 max-w-xl"
            >
            ...
            <motion.div variants={item} className="flex items-center gap-6 sm:gap-8">
              <div className="flex -space-x-3">
                {[1, 2, 3, 4, 5].map((i) => (
                  <div
                    key={i}
                     className="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-gradient-to-br from-primary to-secondary border-2 border-background flex items-center justify-center text-white text-[10px] sm:text-xs font-bold shadow-lg shadow-primary/20"
                  >

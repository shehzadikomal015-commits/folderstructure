    ...
    >
      ...
      >
        ...
        >
          ...
          <p className="text-sm text-success mt-1">Active catalog</p>
        </motion.div>
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <p className="text-sm font-medium text-muted">Total Stock</p>
            <Package className="h-5 w-5 text-muted" />
          </div>
          <p className="text-3xl font-bold text-foreground">
            {allProducts.reduce((sum, p) => sum + (p.stock || 0), 0)}
          </p>
          <p className="text-sm text-muted mt-1">Units available</p>
        </motion.div>
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <p className="text-sm font-medium text-muted">Avg Price</p>
            <Package className="h-5 w-5 text-muted" />
          </div>
          <p className="text-3xl font-bold text-foreground">
            £{allProducts.length > 0 ? (allProducts.reduce((sum, p) => sum + Number(p.price), 0) / allProducts.length).toFixed(2) : "0.00"}
          </p>
          <p className="text-sm text-muted mt-1">Average price</p>
        </motion.div>
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          ...
          <p className="text-sm text-muted mt-1">Product categories</p>
        </motion.div>
      </motion.div>
              ...
              {allProducts.length > 0 ? (
                allProducts.map((product, idx) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <Badge variant="primary">{product.category || "Uncategorized"}</Badge>
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-1">
                        ...
                        <button className="p-2 rounded-lg hover:bg-gray-100 text-muted hover:text-danger transition-colors">
